...
fn main() {
    ...
    let mut card_num:HashMap<u32, u32> = HashMap::new();

    for line in lines {
        let game = process_line(&line);
        let mut matches: u32 = 0;
        for number in game.player_numbers {
            if game.winning_numbers.contains(&number) {
                println!("{} contains {}", game.game_id, number);
                matches += 1;
            }
        }
...
fn clone_cards(card_map: &mut HashMap<u32, u32>, game: &Game, matches: u32){
    card_map.entry(game.game_id).and_modify(|e| *e += 1).or_insert(1);
    for i in 0..matches {
        card_map.entry(game.game_id+i+1).and_modify(|e| *e += 1).or_insert(1);
    }
}
