...
fn main() -> io::Result<()> {
    ...
    for i in 0..7 {
        ...
        translated_seeds = translate(&translated_seeds, &pairings);
...
fn read_initial_seeds(reader: &mut io::BufReader<File>) -> io::Result<Vec<u64>> {
...
fn translate(initial_seeds: &Vec<u64>, mapping: &Vec<Pairings>) -> Vec<u64> {
    let mut translated_seeds:Vec<u64> = Vec::new();
    let mut found: bool;
    for seed in initial_seeds {
        found = false;
        for pair in mapping {
            if seed >= &pair.source_range_start && seed < &(pair.source_range_start + pair.range_length) {
                let translated_seed = seed - pair.source_range_start + pair.destination_range_start;
                translated_seeds.push(translated_seed);
                found = true;
                break;
            }
        }
        if !found {
            translated_seeds.push(*seed);
        }
    }
...
mod tests {
    ...
    #[test]
    fn test_translate() {
        let initial_seeds:Vec<u64> = vec![79, 14, 55, 13];
        let mapping = vec![
            Pairings {
        ...
        let translated_seeds = translate(&initial_seeds, &mapping);
